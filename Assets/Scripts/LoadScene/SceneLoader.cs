...
public class SceneLoader : MonoBehaviour, ISaveable
{
    ...
    [SerializeField]private bool plotLoaded;
    ...
    private void SceneBeginPlot(string plot)
    {
        if(plot == "NULL" || plotLoaded)
        {
            return;
        }
        Debug.Log("开始场景剧情: " + plot);
        DialogSystem.LoadText(plot);
    ...
    private void UpdateSceneIndex()
    {
        ...
        plotLoaded = obj.GetComponent<SceneIndex>().plotLoaded;
        obj.GetComponent<SceneIndex>().plotLoaded = true; // 设置为已加载状态


        plotName = obj.GetComponent<SceneIndex>().plotName;
